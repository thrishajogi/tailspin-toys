...
The home page includes accessible filters for browsing the catalog. Select one or more categories and optionally choose a publisher; the selections can be combined, and the matching cards update immediately in the static page. Use **Clear filters** to restore the full catalog.
...
focused files in [`.github/instructions`](.github/instructions). In particular:

- Comments explain intent, constraints, and non-obvious decisions instead of
  restating code.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc to document their
  purpose, parameters, and return values.
- Reusable Astro components document their typed `Props` interface.
- TypeScript follows the repository's formatting conventions and is checked with
  ESLint for syntax, unused code, and type-aware quality rules.
